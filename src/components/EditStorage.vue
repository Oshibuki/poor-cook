...
const ruleForm: ruleFormObj = reactive({
  id: '',
  name: '',
  category: '',
  description: '',
...
interface ruleFormObj {
  ...
  expirationNumType: string
}

onMounted(() => {
  const currentItem: InventoryItem | undefined = iStore.getInventoryById(query.id as string)
  if (currentItem) {
    ruleForm.id = currentItem.id
    ruleForm.name = currentItem.name
    ruleForm.category = currentItem.category
    ruleForm.description = currentItem.description
    ruleForm.packagingDate = currentItem.packagingDate
    ruleForm.expirationNumber = parseInt(currentItem.expiration.split('|')[0])
    ruleForm.expirationNumType = currentItem?.expiration.split('|')[1]
  }
})
...
const onExpirationConfirm = ({ selectedValues }) => {
  ...
  ruleForm.expirationNumber = selectedValues[0]
...
const saveItem = () => {
  ...
  iStore.findByIdAndUpdate(ruleForm.id, newInventoryItem)
  router.push('/')
}

const deleteItem = () => {
  iStore.findByIdAndDelete(ruleForm.id)
  router.push('/')
}
</script>
...
<template>
  ...
  <van-form>
    <van-cell-group inset>
      ...
      />
      <van-calendar v-model:show="showCalendar" :min-date="minDate" :max-date="today" @confirm="onDateConfirm" />
      <van-field
        v-model="expiration"
        is-link
        readonly
    ...
    <div style="margin: 16px;">
      <van-action-bar>
        <van-action-bar-button color="#79bbff" type="warning" text="保存" @click.stop="saveItem" />
        <van-action-bar-button color="#73767a" type="danger" text="删除" @click.stop="deleteItem" />
      </van-action-bar>
    </div>
  </van-form>
</template>
